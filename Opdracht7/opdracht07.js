...
function averagePrices(){
    ...
    fietsen.forEach(fietsen =>{
        gemmiddelde += fietsen.prijs;
    })

    return Math.round(gemmiddelde / fietsen.length);
}

let buttonGem = document.getElementById("gemmiddelde");
let p = document.createElement("p");

buttonGem.addEventListener("click", function(){
    let textP = document.createTextNode(`De gemmiddelde prijs van de fietsen bedraagt ${averagePrices()}`);
    p.appendChild(textP);
    div.appendChild(p);
});
